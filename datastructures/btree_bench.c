/*
 * btree_bench.c — Asymptotic complexity benchmark for the B-tree.
 *
...
static void bench_insert(BenchResult results[NUM_SIZES])
{
    for (int s = 0; s < NUM_SIZES; s++) {
        ...
        BTree *t = bt_create(cmp_int, NULL, 64);
...
/*
 * Insert (sorted): expected O(log n) per operation — B-tree handles
 * sorted input as well as shuffled.
 */
...
/*
 * Min: expected O(log n) — walks leftmost spine.
 * Calls bt_min n times on a tree of size n.
 */
...
int main(void)
{
    ...
    printf("B-Tree — Asymptotic Complexity Benchmark\n");
    printf("Sizes:");
